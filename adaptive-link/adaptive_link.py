...
import argparse
import os
import configparser  # For reading/writing config files
...
def send_udp(udp_socket, udp_ip, udp_port):
    ...
    while True:
        ...
        try:
            ...
            if verbose_mode:
                print(f"UDP Message Sent: {message} with size {len(message_bytes)} bytes.")
        except Exception as e:
            if verbose_mode:
...
def connect_and_receive_msgpack():
    ...
    # Start a background thread to send the link health score via UDP
